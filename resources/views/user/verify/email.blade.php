          ...
          <div class="card border-0 mb-5">
            <div class="card-body pt-7 px-5">
              <div class="text-center text-dark mb-5">
                ...
                <p>{{__('Verify your Email')}}, <span class="text-muted"><a href="{{route('user.send-emailVcode')}}">{{__('Resend email')}}</a></span></p>
              </div>
              <form role="form" action="{{ route('user.email-verify')}}" method="post" id="payment-form">
                @csrf
                ...
                <div class="text-center">
                  <button type="submit" id="ggglogin" class="btn btn-primary my-4 rounded-pill btn-block">{{__('Verify')}}</button>
                </div>
              </form>
            </div>
          </div>          
