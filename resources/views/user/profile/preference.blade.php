...
@section('mainpage')
@include('user.profile.partials.top', ['title'=>'Preferences'])
    <form action="{{route('user.preferences.update')}}" method="post" enctype="multipart/form-data">
        @csrf
        <div class="card bg-light-primary border-primary  rounded p-6 mb-8 mb-6">
            <div class="card-body px-9 pt-6 pb-4">
                <div class="row mb-6">
                    <div class="col">
                        <div>
                            <p>For additional security and to help check against fraud, we will check the details you provide against
                                the payee account details,including the name on their account. Although, this does not prevent all
                                types of fraud, this will give you reassurance that you are paying the correct person</p>
                        </div>
                    </div>
                </div>
                <div class="row mb-6">
                    <div class="col-lg-12">
                        <input type="email" name="support_email" id="support_email" class="bg-white form-control form-control-lg form-control-solid" placeholder="{{__('Email address')}}" value="{{$user->support_email}}" required>
                    </div>
                </div>
                <div class="row mb-6">
                    <div class="col-lg-12">
                        <div class="input-group input-group-solid bg-white">
                          <span class="input-group-prepend ">
                            <span class="input-group-text ">+{{$user->getCountry()->phonecode}}</span>
                          </span>
                            <input type="tel" name="phone" maxlength="14" class="form-control bg-white form-control-lg form-control-solid" placeholder="{{__('Phone number')}}" value="{{$user->support_phone}}" required>
                        </div>
{{--                        <input type="text" name="support_phone" id="support_phone" class="bg-white form-control form-control-lg form-control-solid" placeholder="{{__('Phone number')}}" value="{{$user->support_phone}}" required>--}}
                    </div>
                </div>
                <div class="row mb-6">
                    <div class="col-lg-12">
                        <select name="checkout_theme" aria-label="Select a theme" data-control="select2" data-placeholder="Select a theme..." class="bg-white form-select form-select-solid form-select-lg select2-hidden-accessible" data-select2-id="select2-data-4-swnc" tabindex="-1" aria-hidden="true">
...
{{--                @endif--}}
            ...
            <div class="card-footer d-flex justify-content-end py-6 px-9">
                <a href="#" data-bs-toggle="modal" data-bs-target="#delete_account" class="btn btn-light-danger btn-active-danger me-2"> {{__('Delete account')}}</a>
    ...
    <div class="card bg-light-primary">
        <div class="card-header d-flex align-items-center justify-content-start py-4 border-0">
            <h4 class="text-dark font-weight-bold">{{__('Mail Configuration')}}</h4>
            <small>Lorem ipsum dolor sit amet, consectetur adipisicing elit. Deserunt, praesentium.</small>
        </div>
        <div class="card-body border-0">
            <div class="row mb-6">
                <label class="col-lg-12 col-form-label required fw-bold fs-6">{{__('What email smtp configuration will you be using?')}}</label>
                <div class="col-lg-12">
                    <select id="mail" name="status" class="form-select form-select-lg bg-white form-select-solid" required>
            ...
            <div class="row mb-6" id="showMail" style="display:none;">
                <div class="col-lg-6">
                    ...
                    <input type="text" placeholder="smtp.com" value="{{($user->mailDriver) ? $user->mailDriver->mail_host : ""}}" class="form-control bg-white form-control-solid" name="mail_host" id="mail_host" required>
                </div>
                <div class="col-lg-6">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail Port')}}</label>
                    <input type="text" placeholder="465" value="{{ ($user->mailDriver) ?  $user->mailDriver->mail_port :"" }}" class="form-control bg-white form-control-solid" name="mail_port" id="mail_port" required>
                </div>
                <div class="col-lg-6">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail Username')}}</label>
                    <input type="text" placeholder="username" value="{{ ($user->mailDriver) ? $user->mailDriver->mail_username :""}}" class="form-control bg-white form-control-solid" name="mail_username" id="mail_username" required>
                </div>
                <div class="col-lg-6">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail Password')}}</label>
                    <input type="password" placeholder="password" value="{{ ($user->mailDriver) ? $user->mailDriver->mail_password :""}}" class="form-control bg-white form-control-solid" name="mail_password" id="mail_password" required>
                </div>
                <div class="col-lg-6">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail Encryption')}}</label>
                    <input type="text" placeholder="SSL/TLS" value="{{ ($user->mailDriver) ? $user->mailDriver->mail_encryption :""}}" class="form-control bg-white form-control-solid" name="mail_encryption" id="mail_encryption" required>
                </div>
                <div class="col-lg-6">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail From Address')}}</label>
                    <input type="email" placeholder="email address" value="{{($user->mailDriver) ? $user->mailDriver->mail_from_address : "" }}" class="form-control bg-white form-control-solid" name="mail_from_address" id="mail_from_address" required>
                </div>
                <div class="col-lg-12">
                    <label class="col-form-label fw-bold fs-6">{{__('Mail From Name')}}</label>
                    <input type="text" placeholder="sender name" value="{{($user->mailDriver) ? $user->mailDriver->mail_from_name : ""}}" class="form-control bg-white form-control-solid" name="mail_from_name" id="mail_from_name" required>
                </div>
            </div>
        </div>
        <div class="card-footer d-flex justify-content-end py-6 px-9 border-0">
            <button type="submit" class="btn btn-primary px-6 mt-lg-n10">{{__('Save changes')}}</button>
...
{{--                <div class="accordion-item bg-light-primary border-primary">--}}
...
{{--                        <div class="accordion-body bg-light-primary border-primary">--}}
{{--                            <div class="card-body">--}}
{{--                                <div class="row mb-6">--}}
{{--                                    <label class="col-lg-12 col-form-label required fw-bold fs-6">{{__('What email smtp configuration will you be using?')}}</label>--}}
{{--                                    <div class="col-lg-12">--}}
...
{{--  </form>--}}
  ...
  <div class="card bg-light-primary mt-6">
    ...
    <form action="{{ (!empty($user->customDomain->domain)) ? (env('APP_ENV') == 'local') ? route('disconnect.domain', ['id'=>$user->id]) : route('edit.domain', ['id' => $user->id]) : ""}}" method="post" class="px-5" enctype="multipart/form-data">
        <div class="card-body">
            @csrf
            <input type="url" class="form-control form-control-lg bg-white form-control-solid mb-3" name="custom_domain" placeholder="{{__('Enter Your Custom Domain')}}" value="{{ (!empty($user->customDomain)) ? $user->customDomain->domain : "" }}" @if(!empty($user->customDomain->domain)) disabled @endif  >
            @if(empty($user->customDomain->domain))
                <button type="submit" class="mb-3 btn btn-primary float-lg-end">{{__('Add your domain')}}</button>
            @elseif(env('APP_ENV') == 'local')
                <button type="submit" class="mb-3 btn btn-danger float-lg-end">{{__('Disconnect custom domain')}}</button>
            @endif
        </div>
    </form>
</div>
