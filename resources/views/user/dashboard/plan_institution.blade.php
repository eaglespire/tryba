...
@section('content')
@php
$dd=App\Models\Transactions::whereref_id($reference)->first();
$xcast="person.png"; 
@endphp
                            ...
                            <div class="card-body">
                                <div class="row align-items-center">
                                    <div class="col text-left">
                                        <h3>{{__('You are about to pay')}}</h3>
                                        <p>{{$set->site_name}}</p>
                                        <p>{{__('Payment reference')}}: {{$reference}}</p>
                                        @if($xf->bank_format=="uk")
                                        <p>{{__('Account number')}}: {{$bank->acct_no}}</p>
                                        <p>{{__('Sort code')}}: {{$bank->routing_number}}</p>
                                        @elseif($xf->bank_format="eur")
                                        <p>{{__('Account number')}}: {{$bank->acct_no}}</p>
                                        @endif
                                        <p>{{__('Amount')}}: {{$xf->coin->symbol.number_format($dd->base_amount,'2')}}</p>
                                        {{--<p>{{__('VAT')}}: {{$xf->coin->symbol.number_format($dd->base_amount*20/100,'2')}}</p>--}}
                                        <p>{{__('Total')}}: {{$xf->coin->symbol.number_format($dd->amount,'2')}}</p>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <div class="container mt--8 pb-5">
        <div class="row justify-content-center">  
            <div class="col-md-12">
                ...
                <div class="row justify-content-center">
                    <a data-toggle="modal" data-target="#create" href="" class="btn btn-primary rounded-pill my-4">Select a different provider <i class="fal fa-angle-right"></i></a>
                </div>
        ...
        <div class="row justify-content-center mt-9">
            <div class="col-md-12">
                <div class="text-center">
                    <h5>{{__('Powered by')}} </h5><img alt="html" style="height:auto;max-width:20%;" src="{{asset('asset/images/SafeConnect_logo.png')}}">
