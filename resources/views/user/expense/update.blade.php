    ...
    <div class="toolbar" id="kt_toolbar">
        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                <h1 class="text-dark fw-bolder my-1 fs-1">{{__('Update Expense')}}</h1>
                <ul class="breadcrumb fw-bold fs-base my-1">
                    ...
                    <li class="breadcrumb-item text-muted">
                        <a href="{{route('allExpense')}}" class="text-muted text-hover-primary">{{__('Expenses')}}</a>
                    </li>
            ...
            <div class="card">
                <div class="card-header card-header-stretch">
                    <div class="card-title d-flex align-items-center">
                        <h3 class="fw-bolder m-0 text-dark">{{__('Update Expense')}}</h3>
                    </div>
                </div>

                <form action="{{ route('update.expense',$expense->id) }}" enctype="multipart/form-data" method="post">
                    @csrf
                    @method('put')
                    <div class="card-body px-9 pt-6 pb-4">
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                            <label class="required fs-5 fw-bold mb-2">{{__('Name')}}</label>
                            <input type="text" class="form-control form-control-solid" placeholder="{{__('Name of expense')}}" value="{{ (old('name')) ? old('name') : $expense->name }}" name="name" required>
                        </div>
                        <div class="row mb-6">
                            ...
                            <div class="col-lg-12">
                              <select name="category" id="category" class="form-select form-select-solid" required>
                                    ...
                                    @foreach($categories as $item)
                                        <option @if($expense->categoryID == $item->id) selected @endif value="{{ $item->id }}">{{ $item->Name }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-12">
                              <select name="subcategory" id="subcategory" class="form-select form-select-solid" required>
                                    ...
                                    @foreach($subcategories as $item)
                                        <option @if($expense->subcategoryID == $item->id) selected @endif value="{{ $item->id }}">{{ $item->Name }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                            ...
                            <input type="number" class="form-control form-control-solid" placeholder="{{__('Amount')}}" value="{{ (old('amount')) ? old('amount') : $expense->amount }}" name="amount" required>
                        </div>
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                            <label class="required fs-5 fw-bold mb-2 mt-5">{{__('Date')}}</label>
                            <input type="text" class="form-control  form-control-solid" placeholder="{{__('Date')}}" value="{{ (old('singleDate')) ? old('singleDate') : $expense->date }}"  name="singleDate" required>
                        </div>
                        <div class="row mb-20">
                            <div class="col-lg-12">
                                <label class="required fs-5 fw-bold mb-2">{{__('Description')}}</label>
                                <input type="hidden" id="quill_html" name="description" value="{{ (old('description')) ? old('description') : $expense->description }}">
                                <div data-toggle="quill" data-quill-placeholder="{{__('Describe your expense')}}">{!! (old('description')) ? old('description') : $expense->description !!}</div>
                            </div>
                        </div>
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <button type="submit" class="btn btn-primary px-6">{{__('Update Expense')}}</button>
                    </div>
                </form>
             </div>
        </div>
