                ...
                <table id="kt_datatable_example_6" class="table align-middle table-row-dashed gy-5 gs-7">
                    <thead>
                        <tr class="fw-bolder fs-6 text-gray-800 px-7">
                            ...
                            <th class="min-w-80px">{{__('Description')}}</th>
                            <th class="min-w-70px">{{__('Created')}}</th>
                        </tr>
                    </thead>
                    <tbody>
                    ...
                    <tr>
                        <td>
                            ...
                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px py-3" data-kt-menu="true">
                                ...
                                <div class="menu-item px-3"><a href="{{ route('AllSubCategoryExpense',$item->id) }}" class="menu-link px-3">{{__('View Sub category')}}</a></div>
                                <div class="menu-item px-3"><a href="{{ route('update.CategoryExpense',$item->id) }}" class="menu-link px-3">{{__('Edit')}}</a></div>
                        ...
                        <td data-href="">{!! $item->Description  !!}</td>
                        <td data-href="">{{ (!empty($item->created_at)) ? $item->created_at->diffforHumans() : "" }}</td>
                    </tr>
                    @endforeach
                    </tbody>
                </table>
                    ...
                    <div class="modal-body mt-4">
                        <div class="card bg-white border-0 mb-0">
                            ...
                            <div class="card-body d-flex px-lg-5 py-lg-5 text-right">
                                ...
                                <form method="post" action="{{ route('delete.CategoryExpense', $item->id) }}">
                                    @csrf
                                    @method('delete')
