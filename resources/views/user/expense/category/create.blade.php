    ...
    <div class="toolbar" id="kt_toolbar">
        <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
            <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
                <h1 class="text-dark fw-bolder my-1 fs-1">{{__('Add Category')}}</h1>
    ...
    <div class="post fs-6 d-flex flex-column-fluid" id="kt_post">
        ...
        <div class="container">
            <div class="card">
                ...
                <form action="{{ route('post.CategoryExpense') }}" enctype="multipart/form-data" method="post">
                    @csrf
                    <div class="card-body px-9 pt-6 pb-4">
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                            <label class="required fs-5 fw-bold mb-2">{{__('Name')}}</label>
                            <input type="text" class="form-control form-control-solid" placeholder="{{__('Name of Category')}}" value="{{ (old('name')) ? old('name') : "" }}" name="name" required>
                        </div>
                        <div class="row mb-20">
                            <div class="col-lg-12">
                    ...
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <button type="submit" class="btn btn-primary px-6">{{__('Add Category')}}</button>
                    </div>
                </form>
             </div>
        </div>
