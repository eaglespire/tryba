            ...
            <div class="card">
                <div class="card-header card-header-stretch">
                    <div class="card-title d-flex align-items-center">
                        <h3 class="fw-bolder m-0 text-dark">{{__('Update budget')}}</h3>
                    </div>
                </div>

                <form action="{{ route('putBudget', $budget->id) }}" enctype="multipart/form-data" method="post">
                    @csrf
                    @method('put')
                    <div class="card-body px-9 pt-6 pb-4">
                            ...
                            <div class="col-lg-12">
                              <select name="month" class="form-select form-select-solid" required>
                                    ...
                                    @foreach($months as $item)
                                        <option @if($budget->month == $item) selected @endif value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-12">
                              <select name="year" class="form-select form-select-solid" required>
                                    @foreach($years as $item)
                                        <option @if($budget->year == $item) selected @endif value="{{ $item }}">{{ $item }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-12">
                              <select name="category" id="category" class="form-select form-select-solid" required>
                                    ...
                                    @foreach($categories as $item)
                                        <option @if($budget->categoryID == $item->id) selected @endif value="{{ $item->id }}">{{ $item->Name }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                            ...
                            <div class="col-lg-12">
                              <select name="subcategory" id="subcategory" class="form-select form-select-solid" required>
                                    ...
                                    @foreach($subcategories as $item)
                                        <option @if($budget->subcategoryID == $item->id) selected @endif value="{{ $item->id }}">{{ $item->Name }}</option>
                                    @endforeach
                              </select>
                            </div>
                        </div>
                        <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                            ...
                            <input type="number" class="form-control form-control-solid" placeholder="{{__('Amount')}}" value="{{ (old('amount')) ? old('amount') : $budget->amount }}" name="amount" required>
                        </div>
                        <div class="row mb-20">
                            <div class="col-lg-12">
                                <label class="required fs-5 fw-bold mb-2">{{__('Description')}}</label>
                                <input type="hidden" id="quill_html" name="description" value="{{ (old('description')) ? old('description') : $budget->description }}">
                                <div data-toggle="quill" data-quill-placeholder="{{__('Describe your budget')}}">{!! (old('description')) ? old('description') : $budget->description !!}</div>
                            </div>
                        </div>
                    </div>
                    <div class="card-footer d-flex justify-content-end py-6 px-9">
                        <button type="submit" class="btn btn-primary px-6">{{__('Update Budget')}}</button>
                    </div>
                </form>
             </div>
        </div>
