...
<div class="toolbar" id="kt_toolbar">
    <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
        <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
            <h1 class="text-dark fw-bolder my-1 fs-1">{{__('Archived Transactions')}}</h1>
            ...
            <div class="card-header border-0 pt-6">
                ...
                <div class="card-toolbar">
                    ...
                    <div class="d-flex justify-content-end" data-kt-user-table-toolbar="base">
                            ...
                            <div class="modal-dialog modal-dialog-centered mw-550px">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body scroll-y my-7">
                                        <form action="{{route('user.archive.transactions.sort')}}" method="post">
                                            @csrf
                                            <div class="mb-10">
                                            ...
                                            <div class="mb-10">
                                                <label class="form-label fs-6 fw-bold">{{__('Payment')}}</label>
                                                <select class="form-select form-select-solid" name="payment">
                                                    <option value="0" @if($payment==0) selected @endif>{{__('All')}}</option>
                                                    <option value="1" @if($payment==1) selected @endif>{{__('Single Pot')}}</option>
                                                    <option value="2" @if($payment==2) selected @endif>{{__('Gig Pot')}}</option>
                                                    <option value="3" @if($payment==3) selected @endif>{{__('Invoice')}}</option>
                                                    <option value="4" @if($payment==4) selected @endif>{{__('Merchant')}}</option>
                                                    <option value="8" @if($payment==8) selected @endif>{{__('Store Order')}}</option>
                                                    <option value="9" @if($payment==9) selected @endif>{{__('Subscription')}}</option>
                                                    <option value="10" @if($payment==10) selected @endif>{{__('Appointment')}}</option>
                                                </select>
                                            </div>
                                            <div class="mb-10">
                        ...
                        <!--end::Filter-->
                        <!--begin::Add user-->
                        <form action="{{route('user.rarchive.transactions')}}" method="post">
                            @csrf
                            @foreach($trans as $val)
                            <input type="hidden" id="pos{{$val->ref_id}}" disabled name="archive[]" value="{{$val->ref_id}}" />
                            @endforeach
                            <input type="hidden" id="add" value="0" />
                            <button type="submit" disabled id="xx" class="btn btn-danger">
                                <!--begin::Svg Icon | path: icons/duotone/Navigation/Plus.svg-->
                                <span class="svg-icon svg-icon-2">
                                    <i class="fal fa-folder"></i>
                                </span>
                                {{__('Remove from Archive')}} <span id="addx"></span>
                            </button>
                        </form>
                        <!--end::Add user-->
                    </div>
