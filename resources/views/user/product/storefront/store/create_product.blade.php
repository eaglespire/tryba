        ...
        <div class="notice d-flex bg-light-info rounded border border-info rounded p-6 mb-8">
            ...
            <div class="d-flex flex-stack flex-grow-1 flex-wrap flex-md-nowrap">
                ...
                <div class="mb-3 mb-md-0 fw-bold">
                    <h4 class="text-gray-800 fw-bolder">{{__('Plan limit')}}</h4>
                    <div class="fs-6 text-dark pe-7">{{__('You are only limited to 4 products on a free plan')}}</div>
                </div>
                <!--end::Content-->
                <!--begin::Action-->
                <a href="{{route('user.package')}}" class="btn btn-info px-6 align-self-center text-nowrap">{{__('Upgrade plan')}}</a>
                <!--end::Action-->
            </div>
            <!--end::Wrapper-->
        </div>
        ...
        <div class="card">
            ...
            <form action="{{route('submit.product')}}" enctype="multipart/form-data" method="post">
                ...
                <div class="card-body px-9 pt-6 pb-4">
                    ...
                    <div class="row mb-20">
                        <div class="col-lg-12">
                            ...
                            <div data-toggle="quill" data-quill-placeholder="{{__('Product specification')}}">@if(session('specification')){!!session('specification')!!}@endif</div>
                        </div>
                    </div>
                    <div class="row mb-20">
                        <div class="col-lg-12">
                            <input type="hidden" id="quill_html2" name="details" value="@if(session('details')){{session('details')}}@endif">
                            <div data-toggle="quill" data-quill-placeholder="{{__('More details about your product')}}">@if(session('details')){!!session('details')!!}@endif</div>
                        </div>
                    </div>
                    <div class="d-flex flex-column mb-5 fv-row fv-plugins-icon-container">
                ...
                <div class="card-footer d-flex justify-content-end py-6 px-9">
                    <button type="submit" @if($user->plan->amount==0)@if($user->productCount()==4) disabled @endif @endif class="btn btn-primary px-6">{{__('Create Product')}}</button>
                </div>
            </form>
        </div>
    </div>
