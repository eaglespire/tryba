            ...
            <div class="row">
                <div class="col-5 hidden-xs">
                    <div class="tt-product-single-img">
                        <div>
                            ...
                            <img id="imageDiv" class="zoom-product" src="{{asset('asset/profile/'.$image[0]->image)}}" data-zoom-image="{{asset('asset/profile/'.$image[0]->image)}}" alt="">
                        </div>
                    </div>
                    <div class="product-images-carousel">
                        <ul id="smallGallery" class="arrow-location-02  slick-animated-show-js">
                            @foreach($image as $k=>$val)
                            <li><a href="#" @if($val->id == $image[0]->id) class="zoomGalleryActive" @endif data-image="{{asset('asset/profile/'.$val->image)}}" data-zoom-image="{{asset('asset/profile/'.$val->image)}}"><img id="galleryimg{{$val->id}}" onclick="productGallery(this.id)" src="{{asset('asset/profile/'.$val->image)}}" alt=""></a></li>
                            @endforeach
                        </ul>
                    </div>
                </div>
                <div class="col-7">
                    <div class="tt-product-single-info">
                        ...
                        <span class="fs-2">{{$product->name}}</span><br>
                        ...
                        <div class="tt-price">
                            ...
                            <span class="new-price font-weight-bold fs-2">{{view_currency($product->currency).number_format($product->amount-(intval($product->amount)*intval($product->discount)/100),2)}}</span>
                        ...
                        <div class="tt-review">
                            <div class="tt-rating">
                                <i class="@if($product->rating()==0 || $product->rating()<1)icon-star-empty @else icon-star @endif"></i>
                                <i class="@if($product->rating()==1 || $product->rating()<2)icon-star-empty @else icon-star @endif"></i>
                                <i class="@if($product->rating()==2 || $product->rating()<3)icon-star-empty @else icon-star @endif"></i>
                                <i class="@if($product->rating()==3 || $product->rating()<4)icon-star-empty @else icon-star @endif"></i>
                                <i class="@if($product->rating()==4 || $product->rating()<5)icon-star-empty @else icon-star @endif"></i>
                            </div>
                        </div>
                        ...
                        @endif
                        <form action="{{route('update.cart',['id'=> $store->store_url])}}" method="post">
                            ...
                            <div class="tt-wrapper mb-5">
                                <ul class="tt-list-btn">
                                    ...
                                    <li><a class="btn-link text-dark" href="{{route('customer.wishlist.add', ['id'=>$store->store_url,'wishlistId'=>$product->id])}}"><i class="icon-n-072"></i>{{__('ADD TO WISH LIST')}}</a></li>
                                    @else
                                    <li><a class="btn-link text-dark" href="{{route('store.customer.login', ['id'=>$store->store_url])}}"><i class="icon-n-072"></i>{{__('ADD TO WISH LIST')}}</a></li>
                                    @endif
                                </ul>
                            </div>
                            <div class="tt-row-custom-01">
                                <a class="mr-3 text-dark fs-6" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u={{route('sproduct.link', ['id'=>$store->store_url,'product'=>$product->ref_id])}}"><img src="{{asset('/')}}asset/new_dashboard/media/svg/brand-logos/facebook-3.svg" class="w-20px mr-2" alt="">{{__('Facebook')}}</a>
                                <a class="mr-3 text-dark fs-6" target="_blank" href="https://twitter.com/intent/tweet?text={{route('sproduct.link', ['id'=>$store->store_url,'product'=>$product->ref_id])}}"><img src="{{asset('/')}}asset/new_dashboard/media/svg/brand-logos/twitter.svg" class="w-20px mr-2" alt="">{{__('Twitter')}}</a>
                                <a class="mr-3 text-dark fs-6" target="_blank" href="https://wa.me/?text={{route('sproduct.link', ['id'=>$store->store_url,'product'=>$product->ref_id])}}"><img src="{{asset('/')}}asset/new_dashboard/media/svg/brand-logos/whatsapp.svg" class="w-20px mr-2" alt="">{{__('Whatsapp')}}</a>
                            </div>
                        </form>
                        <div class="tt-collapse-block">
                            <div class="tt-item">
                                <div class="tt-collapse-title">{{__('REVIEWS')}} ({{ count($reviews) }})</div>
