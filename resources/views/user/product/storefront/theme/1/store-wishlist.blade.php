					...
					<table>
						<tbody>
							...
							<tr>
								...
								<td>
									<h2 class="tt-title">
										<a href="{{route('sproduct.link', ['id'=>$store->store_url,'product'=>$val->product->ref_id])}}">{{$val->product->name}}</a>
									</h2>
								</td>
								<td>
									<div class="tt-price">
								...
								<td>
									<a href="{{route('delete.wishlist', ['id'=> $store->store_url , 'wishlistId' => $val->id ])}}" class="tt-btn-close"></a>
								</td>
							</tr>
							@endforeach
						</tbody>
