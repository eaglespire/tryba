    ...
    <div class="container-fluid d-flex flex-stack flex-wrap flex-sm-nowrap">
        <div class="d-flex flex-column align-items-start justify-content-center flex-wrap me-2">
            ...
            <ul class="breadcrumb fw-bold fs-base my-1">
                ...
                <li class="breadcrumb-item text-dark">{{__('Blog')}}</li>
            </ul>
        </div>
        <div class="d-flex py-2">
            <a href="{{route('add.blog.store')}}" class="btn btn-info me-4"><i class="fal fa-newspaper"></i> {{__('New Article')}}</a>
            <a href="{{route('add.blog.cat.store')}}" class="btn btn-dark me-4"><i class="fal fa-folder"></i> {{__('New Category')}}</a>
        </div>
    </div>
</div>
    ...
    <div class="container">
        <div class="d-flex overflow-auto mb-10">
            <ul class="nav nav-stretch nav-line-tabs w-100 nav-line-tabs-2x fs-4 nav-stretch fw-bold">
                <li class="nav-item">
                    <a href="{{route('website.blog')}}" class="nav-link text-active-primary px-3 @if(route('website.blog')==url()->current()) active @endif">{{_('Articles')}}</a>
                </li>
                <li class="nav-item">
                    <a href="{{route('customer.blog.cat')}}" class="nav-link text-active-primary px-3 @if(route('customer.blog.cat')==url()->current()) active @endif">{{__('Category')}}</a>
                </li>
            </ul>
        </div>
        <div class="row g-xl-8">
            <div class="col-xl-12">
                @if(route('website.blog')==url()->current())
                            ...
                            <table id="kt_datatable_example_6" class="table align-middle table-row-dashed gy-5 gs-7">
                                <thead>
                                    <tr class="fw-bolder fs-6 text-gray-800 px-7">
                                        ...
                                        <th class="min-w-80px">{{__('Category')}}</th>
                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td>{{$val->title}}</td>
                                        <td>
                                            <button data-clipboard-text="{{route('store.blog.view', ['id'=>$user->storefront()->id, 'ref'=>$val->id, 'slug'=>$val->slug])}}" class="castro-copy btn btn-active-color-primary btn-color-dark btn-icon btn-sm btn-outline-light">
                ...
                @endif
                @if(route('customer.blog.cat')==url()->current())
                <div class="row g-5 g-xxl-8">
                    <div class="card">
                        <div class="card-body pt-3">
                            <table id="kt_datatable_example_6" class="table align-middle table-row-dashed gy-5 gs-7">
                                ...
                                <tbody>
                                    @foreach(getStoreBlogCat($user->storefront()->id) as $k=>$val)
                                    <tr>
                                        <td>
                                            <button type="button" class="btn btn-clean btn-sm btn-icon btn-light-primary btn-active-light-primary me-n3" data-kt-menu-trigger="click" data-kt-menu-placement="bottom-end" data-kt-menu-flip="top-end">
                                                <span class="svg-icon svg-icon-3 svg-icon-primary">
                                            ...
                                            <div class="menu menu-sub menu-sub-dropdown menu-column menu-rounded menu-gray-800 menu-state-bg-light-primary fw-bold w-200px py-3" data-kt-menu="true">
                                                ...
                                                <div class="menu-item px-3"><a href="{{route('edit.blog.cat.store', ['id'=>$val->id])}}" class="menu-link px-3">{{__('Edit')}}</a></div>
                    ...
                    <div class="modal fade" tabindex="-1" id="cat_delete{{$val->id}}">
                        <div class="modal-dialog">
                            <div class="modal-content">
                                ...
                                <div class="modal-footer">
                                    <button type="button" class="btn btn-light" data-bs-dismiss="modal">Close</button>
                                    <a href="{{route('delete.blog.cat.store', ['id'=>$val->id])}}" class="btn btn-primary">{{__('Proceed')}}</a>
                                </div>
                            </div>
                        </div>
                    </div>
