          ...
          <tbody>  
            @foreach($savings as $k=>$val)
              <tr>
                ...
                <td>@if($val->user['id']==null) [Deleted] @else{{view_currency2($val->user->coin['coin_id']).number_format($val->amount, 2, '.', '')}} @endif</td>
                <td>{{$val->ref_id}}</td>
                <td>{{$val->purpose}}</td>
                <td>@if($val->user['id']==null) [Deleted] @else{{view_currency2($val->user->coin['coin_id']).number_format($val->profit, 2, '.', '')}} @endif</td>
