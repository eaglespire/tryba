                        ...
                        <table class="table table-flush" id="datatable-buttons">
                            ...
                            @foreach($transfer as $k=>$val)
                                <tr>
                                    ...
                                    <td>{{view_currency2($val->from_currency).number_format($val->amount, 2, '.', '')}}</td>
                                    <td>{{view_currency2($val->from_currency).number_format($val->charge, 2, '.', '')}}</td>
                                    <td>
                                        @if($val->status==0)
                                            <span class="badge badge-danger badge-pill">{{__('Pending')}}</span>
                                        @elseif($val->status==1)
                                            <span class="badge badge-success badge-pill">{{__('Successful')}}</span>                                        
                                        @elseif($val->status==2)
                                            <span class="badge badge-primary badge-pill">{{__('Returned')}}</span> 
                                        @endif
                                    </td>     
