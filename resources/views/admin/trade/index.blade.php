                ...
                <tbody>
                    @foreach($trx as $k=>$val)
                        <tr>
                            ...
                            <td class="">
                                <div class="dropdown">
                                    ...
                                    <div class="dropdown-menu dropdown-menu-right dropdown-menu-arrow">
                                        @if($val->status==0)
                                            <a class='dropdown-item' href="{{route('trade.approve', ['id' => $val->id])}}"><i class="fad fa-check"></i> {{__('Approve request')}}</a>
                                            <a class='dropdown-item' href="{{route('trade.decline', ['id' => $val->id])}}"><i class="fad fa-ban"></i> {{__('Decline request')}}</a>
                                        @endif
                                        @if($val->type==1 || $val->type==4)
                                        <a data-toggle="modal" data-target="#wallet{{$val->id}}" href="" class="dropdown-item"><i class="fad fa-wallet"></i> {{__('Wallet')}}</a>
                                        @endif
                            ...
                            <td>{{$val->user->coin->bb->symbol.number_format($val->amount, 2, '.', '')}}</td>
                            @endif
                            @if($val->type==1 || $val->type==4)                           
                            <td>{{$val->user->coin->bb->symbol.number_format($val->total, 2, '.', '')}}</td>
                            ...
                            @elseif($val->status==2)
                                <span class="badge badge-primary badge-pill">{{__('Declined')}}</span> 
                            @endif
                        </td>
