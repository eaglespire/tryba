            ...
            <table class="table table-flush" id="datatable-buttons">
                ...
                @foreach($ticket as $k=>$val)
                    <tr>
                        ...
                        <td>
                            @if($val->status==0)
                                <span class="badge badge-pill badge-primary">{{__('Open')}} | {{$val->priority}}</span>
                            @elseif($val->status==1)
                                <span class="badge badge-pill badge-danger">{{__('Closed')}} | {{$val->priority}}</span>                                        
                            @elseif($val->status==2)
                                <span class="badge badge-pill badge-success">{{__('Resolved')}} | {{$val->priority}}</span> 
                            @endif
                        </td>   
