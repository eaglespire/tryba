...
$(document).ready(function(e) {
  ...
  //   $("#mccModal").modal({show: true, backdrop: 'static'})
    ...
    function getNewAccessToken() {
        ...
        let business_type = document.querySelector('#business_type');
        ...
        let sixthpageLimited = document.querySelector("#sixth-page-limited");
        let sixthLimitedBtn = document.querySelector('#sixth-page-limited-btn');
        let sixthLimitedBack = document.querySelector("#sixth-page-limited-back");
        ...
        let tenthPageLimited = document.querySelector("#tenth-page-limited");
        let tenthBtnLimited = document.querySelector("#tenth-btn-limited");
        let tenthBackLimited = document.querySelector("#tenth-back-limited");

        let eleventhPageLimited = document.querySelector("#eleventh-page-limited");
        let eleventhBtnLimited = document.querySelector("#eleventh-btn-limited");
        let eleventhBackLimited = document.querySelector("#eleventh-back-limited");

        let twelfthPageLimited = document.querySelector("#twelfth-page-limited");
        let twelfthBtnLimited = document.querySelector("#twelfth-btn-limited");
        let twelfthBackLimited = document.querySelector("#twelfth-back-limited");

        let seventhPageSoletrader = document.querySelector("#seventh-page-soletrader");
        let seventhBtnSoletrader = document.querySelector("#seventh-btn-soletrader");
        let seventhBackSoletrader = document.querySelector("#seventh-back-soletrader");
        ...
        fouthBtn.addEventListener('click',() => {
          let turnover = document.querySelector('#turnover').value;
          let category = document.querySelector("#business_category").value
          if(!turnover){
            errorMsg.innerText = "Enter an amount";
          }else if(!category){
            errorMsg.innerText = "Select a merchant category";
          }else{
              errorMsg.innerText = "";
              fouthPage.classList.toggle('d-none');
        ...
        fifthBtn.addEventListener('click',() => {
            ...
            let soletrader_business_name = document.querySelector("#soletrader_business_name").value;
            let modalSize = document.querySelector('#modal-size');
            if(!business_type.value){
              errorMsg.innerText = "Business type field is required";
            }else if (!value && business_type.value == "LLC") {
                errorMsg.innerText = "Business name field is required";
            }else if(!soletrader_business_name && business_type.value != "LLC"){
              errorMsg.innerText = "Business name field is required";
            }else if(business_type.value == "LLC"){
              modalSize.classList.remove('modal-md')
              modalSize.classList.add('modal-lg')
              errorMsg.innerText = "";
              fifthPage.classList.toggle('d-none');
              sixthpageLimited.classList.toggle('d-none');
            }else{
              errorMsg.innerText = "";
              fifthPage.classList.toggle('d-none');
        ...
        sixthLimitedBtn.addEventListener('click',()=>{
            let modalSize = document.querySelector('#modal-size');
            seventhPage.classList.toggle('d-none');
            sixthpageLimited.classList.toggle('d-none');
            modalSize.classList.add('modal-md')
            modalSize.classList.remove('modal-lg')
            errorMsg.innerText = "";
        })

        sixthLimitedBack.addEventListener('click',()=>{
            let modalSize = document.querySelector('#modal-size');
            modalSize.classList.add('modal-md')
            modalSize.classList.remove('modal-lg')
            errorMsg.innerText = "";
        ...
        eightBtn.addEventListener('click',() => {
            ...
            let websiteURL = document.querySelector("#websiteURL").value ? document.querySelector("#websiteURL").value : false;
            let facebookURL = document.querySelector("#facebookURL").value ? document.querySelector("#facebookURL").value : false;
            let instagramURL = document.querySelector("#instagramURL").value ? document.querySelector("#instagramURL").value : false;
            let twitterURL = document.querySelector("#twitterURL").value ? document.querySelector("#twitterURL").value : false;
            let regex = /(?:www?)(\w+:?\w*).?(\S+)(:\d+)?(\/|\/([\w#!:.?+=&%!\-\/]))?/;
            // if(!value){
            //     errorMsg.innerText = "Select an option";
            // }else
            // if(value == 'yes' && !websiteURL){
            //   errorMsg.innerText = "Website URL field is required";
            // } else
            if(value == 'yes' && websiteURL !== ' ' && !regex.test(websiteURL)) {
              errorMsg.innerText = "Please provide a valid website url starting with www.";
            } else if(value === 'no' && facebookURL !== ' ' && !regex.test(facebookURL)) {
              errorMsg.innerText = "Please provide a valid Facebook url starting with www.";
            } else if(value === 'no' && instagramURL !== ' ' && !regex.test(instagramURL)) {
              errorMsg.innerText = "Please provide a valid Instagram url starting with www.";
            } else if(value === 'no' && twitterURL !== ' ' && !regex.test(twitterURL)) {
              errorMsg.innerText = "Please provide a valid Twitter url starting with www.";
            } else{
              errorMsg.innerText = "";
              if(business_type.value == 'LLC'){
                eightPage.classList.toggle('d-none');
                ...
                seventhPageSoletrader.classList.toggle('d-none');
                eightPage.classList.toggle('d-none');
              }
            }
        });

        seventhBtnSoletrader.addEventListener('click',()=>{
            let value = (document.querySelector('input[name="gender"]:checked')) ? document.querySelector('input[name="gender"]:checked').value : false;
            let soletraderAddress1 = document.querySelector('#soletraderAddress1') ? document.querySelector('#soletraderAddress1').value : false;
            let soletraderAddress2 = document.querySelector('#soletraderAddress2') ? document.querySelector('#soletraderAddress2').value : false;
            let soletraderCity = document.querySelector('#soletraderCity') ? document.querySelector('#soletraderCity').value : false;
            let soletraderPostCode = document.querySelector('#soletraderPostCode') ? document.querySelector('#soletraderPostCode').value : false;
            ...
            } else if(!soletraderAddress1){
              errorMsg.innerText = "Please provide Address 1";
            } else if(!soletraderAddress2){
              errorMsg.innerText = "Please provide Address 2";
            } else if(!soletraderCity){
              errorMsg.innerText = "Please provide City information";
            } else if(!soletraderPostCode){
              errorMsg.innerText = "Please provide Post Code";
            } else{
              errorMsg.innerText = "";
              let date = `${year}/${month}/${day}`
              seventhPageSoletrader.classList.toggle('d-none');
              eleventhPageLimited.classList.toggle('d-none');
            }
        })

        seventhBackSoletrader.addEventListener('click',()=>{
          errorMsg.innerText = "";
          seventhPageSoletrader.classList.toggle('d-none');
          eightPage.classList.toggle('d-none');
        })
        ...
        ninthBtn.addEventListener('click',() => {
            ninthPage.classList.toggle('d-none');
            tenthPageLimited.classList.toggle('d-none');
        });
        ...
        tenthBtnLimited.addEventListener('click',()=>{
            let value = (document.querySelector('input[name="is_user_director"]:checked')) ? document.querySelector('input[name="is_user_director"]:checked').value : false;
            if(!value){
              errorMsg.innerText = "Select an option";
            }else if(value == "no"){
              errorMsg.innerText = "Sorry, but only a director can create an account";
            }else{
              errorMsg.innerText = ""
              eleventhPageLimited.classList.toggle('d-none');
              tenthPageLimited.classList.toggle('d-none');
        ...
        tenthBackLimited.addEventListener('click',()=>{
          errorMsg.innerText = ""
          ninthPage.classList.toggle('d-none');
          tenthPageLimited.classList.toggle('d-none');
        })

        eleventhBtnLimited.addEventListener('click',()=>{
          errorMsg.innerText = ""
          eleventhPageLimited.classList.toggle('d-none');
          twelfthPageLimited.classList.toggle('d-none')
          modalSize.classList.remove('modal-md')
          modalSize.classList.add('modal-lg')
        })

        eleventhBackLimited.addEventListener('click',()=>{
          errorMsg.innerText = ""
          if(business_type.value == 'LLC'){
            eleventhPageLimited.classList.toggle('d-none');
            tenthPageLimited.classList.toggle('d-none');
        ...
        twelfthBtnLimited.addEventListener('click', ()=>{
            modalSize.classList.add('modal-md')
            modalSize.classList.remove('modal-lg')
            twelfthPageLimited.classList.toggle('d-none')
        ...
        twelfthBackLimited.addEventListener('click',()=>{
            modalSize.classList.add('modal-md')
            modalSize.classList.remove('modal-lg')
            twelfthPageLimited.classList.toggle('d-none')
            eleventhPageLimited.classList.toggle('d-none');
        ...
        finalBtn.addEventListener('click',() => {
            errorMsg.innerText = "";
            $(".spinnerz").show()
            let formdata = Object.fromEntries(new FormData($("#accOpeningForm").get(0)).entries());
            if(formdata.business_type === 'LLC') {
                let prevData = JSON.parse(localStorage.getItem('formdata'))
                console.log("prevData", prevData)
                formdata.name = formdata.business_type === 'LLC' ? prevData.name : ''
                formdata.associates = prevData.associates
                formdata.tradingAddress = prevData.tradingAddress
                formdata.registeredAddress = prevData.registeredAddress
            }
            formdata.dob = `${formdata.year_account}-${formdata.month_account}-${formdata.date_account}`
            if(formdata.business_type === 'SOLETRADER' || formdata.business_type === 'INDIVIDUAL') {
                let label = 'Self Employed ';
                $(".c-type").text(formdata.business_type+' - '+label)
                $(".ex-monthly").text(formdata.turnover)
                $(".c-name").text(formdata.soletrader_business_name)
            }
            ...
            .then(response => {
                if(response.data.status && response.data.token != null) {
                    $('#accOpeningForm').addClass('d-none');
                    $('#verificationModal').modal('show')
        ...
        // $('input[name="istherestaff"]').on('change', function(e) {
        //   var item = e.target.value;
        //   errorMsg.innerText = "";
        //   let div = document.querySelector("#staff_Details");
        //   if(item == 'yes'){
        //       div.classList.toggle('d-none');
        //   }else{
        //       div.classList.add('d-none');
        //   }
        // });
        $('input[name="is_address_same"]').on('change', function(e) {
            var item = e.target.value;
            errorMsg.innerText = "";
            let div = document.querySelector("#owner_details");
            if(item == 'no'){
                div.classList.toggle('d-none');
            }else{
                div.classList.add('d-none');
        ...
        $('.business_name').select2({
            ajax: {
                url: '/api/compliance-verification/company-data',
                ...
                processResults: function (data) {
                    companyData = data
                    console.log("DATA", data)
                    let arr = $.map(data, function (obj) {
                        obj.id = obj.id || obj.company_number;
    ...
    $(".business_name").on("change", function(e) {
        e.stopImmediatePropagation();
        let companyReg = $(this).val()
        console.log("ComoanyData", companyData)
        let arr = Object.keys(companyData).map((key) => companyData[key]);
        let ar = arr.filter(company => company.id == companyReg)
        selectCompany = ar[0]
        $(".c-name").text('')
        $(".c-reg").text('')
        $(".reg-address").text('')
        $(".trading-address").text('')
        $(".officers").text('')
        $(".c-reg").text(ar[0].id)
        $(".c-name").text(ar[0].title)
        $(".reg-address").text(ar[0].address_snippet)
        axios.get(`/api/compliance-verification/company-data/${companyReg}`)
        .then(response => {
            console.log("res", response.data)
            let formdata = Object.fromEntries(new FormData($("#accOpeningForm").get(0)).entries());
            formdata.companyRegNumber = ar[0].company_number
            formdata.name = ar[0].title
            formdata.country_code = 'GB'
            // getOfficers(ar[0].company_number, response.data.etag)
            formdata.registeredAddress = {
                "addressLine1": ar[0].address.address_line_1,
                "addressLine2": ar[0].address.address_line_2,
                "country": "GB",
                "postCode": ar[0].address.postal_code,
                "postTown": ar[0].address.locality
            }
            ...
            let tradingAddress = formdata.tradingAddress.addressLine1
            $(".trading-address").text(tradingAddress)
            // console.log("FORM ==>", formdata)
            let htm = ''
            let dContent = ''
            let associates = [];
            $(".officers").replaceWith(`<div class="mt-4 officers" id="officers"></div>`)
            $.each(response.data.items, function(index, officer) {
                console.log("officer", officer)
              htm =  `
                    <div class="mt-4">
                        <div class="fw-boldest fs-6 officer-name">${officer.officer_role}</div>
                            <div class="mt-4">
                                <div class="border-brand rounded p-2">
                                    <div class="d-flex justify-content-between align-items-center">
                                        <div class="icon-circle rounded-circle bg-brand-opacity-4  d-flex justify-content-center align-items-center ">
                                            <img src="{{ asset('asset/new_dashboard/media/svg/user.svg') }}" alt="">
                                        </div>
                                        <div class="fw-boldest">
                                            ${officer.name}
                                        </div>
                                        <div class="fw-boldest">
                ...
                $(".officers").append(htm)
                if(index != 0) {
                    dContent = `<div class="sf-details mt-3">
                        <h5>${officer.name}'s Address</h5>
                        <div class="row">
                            <div class="col-md-6 mt-4">
                                <input type="tel" name="phoneNumber_${index}" class="form-control form-control-solid" placeholder="Phone Number">
                            </div>
                            <div class="col-md-6 mt-4">
                                <input type="date" name="dob_${index}" class="form-control form-control-solid" placeholder="Date of birth">
                            </div>
                        </div>
                        <div class="row">
                            <div class="mt-4 col-md-6">
                                <input type="text" name="address_line1_${index}" class="form-control form-control-solid" placeholder="Address 1">
                            </div>
                            <div class="mt-4 col-md-6">
                                <input type="text" name="address_line2_${index}" class="form-control form-control-solid" placeholder="Address 2">
                            </div>
                        </div>
                        <div class="row">
                            <div class="mt-4 col-md-6">
                                <input type="text" name="city_${index}" class="form-control form-control-solid" placeholder="City">
                            </div>
                            <div class="mt-4 col-md-6">
                                <input type="text" name="postcode_${index}" class="form-control form-control-solid" placeholder="Postcode">
                            </div>
                        </div>
                        <div class="row">
                            <div class="mt-4 col-md-6">
                                <input type="text" name="email_${index}" class="form-control form-control-solid" placeholder="Postcode">
                            </div>
                            <div class="mt-4 col-md-6">
                                <input type="text" name="ownership_${index}" class="form-control form-control-solid" placeholder="Share size">
                            </div>
                        </div>
                    </div>`;
                    $(".staff_Details").append(dContent);
                }

            })
            $.each(response.data.items, function(index, officer) {
                let first_name = officer.name.split(',')[1]
                let last_name = officer.name.split(',')[0]
                associates[index] =
                {
                    "applicant": index == 0 ? 1 : 0,
                    "type": index == 0 ? 'DIRECTOR' : 'BENE_OWNER' ,
                    "firstName": first_name,
                    "lastName": last_name,
                    "dateOfBirth": '', //officer.date_of_birth.year+'-'+officer.date_of_birth.month+'-'+'01',
                    "email": '',
                    "phone": '',
                    "homeAddress": {
                        "addressLine1": officer.address.address_line_1,
                        "addressLine2": officer.address.address_line_2,
                        "country": "GB",
                        "postCode": officer.address.postal_code,
                        "postTown": officer.address.locality
                    }
                }
    ...
    $("#business_type").on("change", function(e) {
        ...
        let formdata = Object.fromEntries(new FormData($("#accOpeningForm").get(0)).entries());
        let value = $(this).val()
        let label = value === 'LLC' ? 'Limited Liability Company' : 'Self Employed ';
        formdata.type = value
        $(".c-type").text(value+' - '+label)
        $(".ex-monthly").text(formdata.turnover)
        // console.log("FOORM", formdata)
        if(value == 'LLC'){
          $("#soletrader_div").addClass('d-none');
          $("#business_name_div").removeClass('d-none')
        }else if( value == "SOLETRADER" || value == "INDIVIDUAL") {
          $("#soletrader_div").removeClass('d-none');
    ...
    $("#eleventh-btn-limited").on("click", function(e) {
        e.stopImmediatePropagation()
        // $(".c-type").text('')
        // $(".ex-monthly").text('')
        let formdata = Object.fromEntries(new FormData($("#accOpeningForm").get(0)).entries());
        if(formdata.business_type === 'SOLETRADER' || formdata.business_type === 'INDIVIDUAL') {
            ...
            formdata.homeAddress = {
                "addressLine1": formdata.soletrader_addresss_line1,
                "addressLine2": formdata.soletrader_addresss_line2,
                "country": "GB",
                "postCode": formdata.soletrader_postalcode,
                "postTown": formdata.soletrader_city
            }
            formdata.tradingAddress = {
                "addressLine1": formdata.soletrader_addresss_line1,
                "addressLine2": formdata.soletrader_addresss_line2,
           ...
           let htm =  `
            <div class="mt-4">
                <div class="fw-boldest fs-6">SOLE TRADER</div>
                    <div class="mt-4">
                        <div class="border-brand rounded p-2">
                            <div class="d-flex justify-content-between align-items-center">
                                <div class="icon-circle rounded-circle bg-brand-opacity-4  d-flex justify-content-center align-items-center ">
    ...
    // $("#eight-btn").on("click", function(e) {
    //     e.stopImmediatePropagation()
    //     let htm = '';

    // })

    function getOfficers(companyReg, id) {
        axios.get(`/api/compliance-verification/company-data/${companyReg}/${id}/officers`)
        .then(response => {
